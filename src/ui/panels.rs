...
use crate::utils;
use egui::{self, Color32, Rect, RichText, ScrollArea, Stroke, StrokeKind, Vec2};
...
pub fn render_sidebar(
    ...
    ui.horizontal(|ui| {
        ...
        ui.painter()
            .rect_filled(rect, egui::CornerRadius::same(10), COLOR_PRIMARY);
        ui.painter().text(
            rect.center(),
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ...
            let project_name = config
                .active_project()
                .map(|p| p.name.clone())
                .unwrap_or("Select Project".to_string());
            ...
            ui.menu_button(
                ...
                |ui| {
                    for project in &config.projects {
                        if ui
                            .selectable_label(
                                config.active_project_id.as_ref() == Some(&project.id),
                                &project.name,
                            )
                            .clicked()
                        {
    ...
    for (tab, icon, label) in tabs {
        ...
        let (rect, response) =
            ui.allocate_exact_size(Vec2::new(width - 12.0, 40.0), egui::Sense::click());
        ...
        if ui.is_rect_visible(rect) {
            ...
            if is_active {
                ...
                ui.painter().rect_stroke(
                    ...
                    Stroke::new(1.0, COLOR_SIDEBAR_BORDER),
                    StrokeKind::Inside,
                );
                ui.painter().rect_filled(
                    Rect::from_min_size(
                        rect.left_center() + Vec2::new(4.0, -8.0),
                        Vec2::new(3.0, 16.0),
                    ),
    ...
    ui.with_layout(egui::Layout::bottom_up(egui::Align::LEFT), |ui| {
        ...
        ui.horizontal_centered(|ui| {
            ...
            ui.add_space(8.0);
            ui.label(
                RichText::new(format!("SYSTEM STATUS: {}", status_text))
                    .size(9.0)
                    .strong()
                    .color(COLOR_TEXT_MUTED),
            );
...
pub fn render_dashboard(
    ...
    egui::Grid::new("system_wellness_grid")
        ...
        .show(ui, |ui| {
            ...
            stat_card(
                ...
                &format!(
                    "{:.1}GB",
                    sys_stats.memory_used as f64 / 1024.0 / 1024.0 / 1024.0
                ),
                "💾",
                COLOR_SECONDARY,
            );
    ...
    ui.columns(2, |columns| {
        columns[0].vertical(|ui| {
            ui.label(
                RichText::new("WORKSPACE CONTEXT")
                    .size(9.0)
                    .color(COLOR_TEXT_MUTED)
                    .strong()
                    .extra_letter_spacing(1.2),
            ...
            card_frame(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.vertical(|ui| {
                        if let Some(project) = config.active_project_mut() {
                            ...
                            ui.label(
                                RichText::new(&project.directory)
                                    .size(11.0)
                ...
                if let Some(project) = config.active_project_mut() {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if ui
                            .add(
                                egui::TextEdit::singleline(&mut project.domain)
                                    .desired_width(120.0),
                            )
                            .changed()
                        {
                    ...
                    ui.horizontal(|ui| {
                        if ui
                            .add(
                                egui::Button::new(RichText::new("🔗  Open").strong())
                                    .fill(COLOR_BG_HOVER),
                            )
                            .clicked()
                        {
                        ...
                        if ui
                            .add(
                                egui::Button::new(RichText::new("📂  Explore").strong())
                                    .fill(COLOR_BG_HOVER),
                            )
                            .clicked()
                        {
        ...
        columns[1].vertical(|ui| {
            ...
            card_frame(ui, |ui| {
                ...
                ui.horizontal_centered(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.label(
                        RichText::new(if docker_available {
                            "Daemon is Online"
                        } else {
                            "Daemon Offline"
                        })
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(8.0);
        ui.label(
            RichText::new("SERVICE STACK OVERVIEW")
                .size(9.0)
                .color(COLOR_TEXT_MUTED)
                .strong()
                .extra_letter_spacing(1.2),
    ...
    if let Some(project) = config.active_project() {
        ...
        if enabled_services.is_empty() {
            ui.label(
                RichText::new("No services enabled in this stack.")
                    .color(COLOR_TEXT_MUTED)
                    .italics(),
            );
        } else {
    ...
    egui::Frame::new()
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal_centered(|ui| {
                ...
                // Glow effect
                ui.painter()
                    .circle_filled(rect.center(), 24.0, accent.gamma_multiply(0.1));
                ui.painter().circle_stroke(
                    rect.center(),
                    20.0,
                    Stroke::new(1.0, accent.gamma_multiply(0.2)),
                );
            ...
            ui.painter().rect_filled(
                Rect::from_min_size(
                    rect.left_bottom() + Vec2::new(12.0, -4.0),
                    Vec2::new(rect.width() - 24.0, 3.0),
                ),
                egui::CornerRadius::same(1),
                accent,
            );
...
fn service_card_compact(
    ...
    egui::Frame::new()
        ...
        .stroke(Stroke::new(
            1.0,
            if running {
                COLOR_PRIMARY.gamma_multiply(0.4)
            } else {
                COLOR_BORDER
            },
        ))
        ...
        .show(ui, |ui| {
            ...
            ui.horizontal_centered(|ui| {
                ...
                ui.painter()
                    .rect_filled(rect, egui::CornerRadius::same(10), COLOR_BG_PANEL);
                ui.painter().rect_stroke(
                    rect,
...
pub fn render_services(ui: &mut egui::Ui, config: &mut AppConfig, containers: &[ContainerInfo]) {
    let mut something_changed = false;
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(8.0);
        ui.label(
            RichText::new("SERVICE STACK CONFIGURATION")
                .size(10.0)
                .color(COLOR_TEXT_MUTED)
                .strong()
                .extra_letter_spacing(1.2),
        ...
        if let Some(project) = config.active_project() {
            ui.label(
                RichText::new(format!("({})", project.services.len()))
                    .size(10.0)
                    .color(COLOR_TEXT_DIM),
            );
        }
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if ui
                .add(
                    egui::Button::new(
                        RichText::new("➕ Add Custom Service")
                            .strong()
        ...
        egui::Grid::new("container_list")
            ...
            .show(ui, |ui| {
                ...
                for c in containers {
                    ...
                    ui.label(RichText::new(&c.state).size(13.0).color(if running {
                        COLOR_SUCCESS
                    } else {
                        COLOR_TEXT_DIM
                    }));
                    ui.label(
                        RichText::new(utils::truncate_string(&c.ports, 50))
                            .size(11.0)
                            .color(COLOR_TEXT_DIM),
                    );
                    ui.end_row();
...
pub fn render_logs(ui: &mut egui::Ui, logs: &[String], clear_logs: &mut bool) {
    ...
    ui.horizontal(|ui| {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if ui
                .button(RichText::new("🗑 Clear Output").size(12.0))
                .clicked()
            {
                *clear_logs = true;
            }
...
pub fn render_terminal(
    ...
    ui.horizontal(|ui| {
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            if !terminal_running {
                if ui
                    .button(RichText::new("▶ Start Shell").color(COLOR_SUCCESS))
                    .clicked()
                {
                    *start_terminal = true;
                }
            } else if ui
                .button(RichText::new("⏹ Reset").color(COLOR_ERROR))
                .clicked()
            { /* logic to kill */
            }
...
pub fn render_ports(ui: &mut egui::Ui, port_infos: &[PortInfo], scan_ports: &mut bool) {
    ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.label(
            RichText::new("Detect conflicts before starting your services")
                .size(14.0)
                .color(COLOR_TEXT_DIM),
        );
        ui.add_space(24.0);
            ...
            .show(ui, |ui| {
                ...
                for info in port_infos {
                    ...
                    if info.in_use {
                        ...
                        ui.label(
                            RichText::new(&info.process)
                                .size(12.0)
                                .color(COLOR_TEXT_DIM),
                        );
                        ui.label(
                            RichText::new("Kill / Change Port")
                                .size(12.0)
                                .color(COLOR_WARNING),
                        );
                    } else {
...
pub fn render_settings(
    ...
    ScrollArea::vertical().show(ui, |ui| {
        ...
        card_frame(ui, |ui| {
            ...
            ui.label(
                RichText::new(
                    "Generate locally trusted certificates for your development domains.",
                )
                .color(COLOR_TEXT_DIM),
            );
            ui.add_space(8.0);
            ui.horizontal(|ui| {
                if ui.button("Generate Certs").clicked() {
                    *gen_ssl = true;
                }
                if ui.button("Remove Certs").clicked() {
                    *rem_ssl = true;
                }
            });
        });
